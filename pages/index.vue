<template>
  <div class="flex bg-twitter-bg">
    <div
      class="sm:flex hidden justify-start sm:justify-end w-1/12 md:w-2/12 xl:w-3/12 2xl:w-4/12 h-screen"
    ></div>
    <left-side class="fixed left-0 md:-left-10"></left-side>
    <div
      class="flex xl:w-9/12 w-full text-white border-r-2 sm:border-l-2 border-gray-700"
    >
      <div class="flex flex-col w-6/12 flex-grow">
        <div
        ...
        <!-- div -->
        <div class="h-3 w-full bg-twitter-gray border-b border-gray-700"></div>

        <tweet v-for="i in 12" :key="i"></tweet>
      </div>
      <div
        class="lg:flex xl:w-6/12 hidden flex-grow w-5/12 bg-yellow-400"
      ></div>
    </div>
  </div>
</template>
...
export default {
  components: { LeftSide, ResizeAuto, TweetSection, Tweet },
  head() {
    return {
